...
void wr16(uint32_t address, uint16_t parameter)
{
  ...
  //RAM_REG = 0x302000 and high bit is set - result always 0xB0
  HAL_SPI_Write((uint8_t)((address >> 16) | 0x80));
  //Next byte of the register address   
  HAL_SPI_Write((uint8_t)(address >> 8));
  //Low byte of register address - usually just the 1 byte offset
  HAL_SPI_Write((uint8_t)address);
  //Little endian (yes, it is most significant bit first and least significant byte first)
  HAL_SPI_Write((uint8_t)(parameter & 0xFF));
...
{
  sendCmd(CMD_SLIDER);
  sendCmd( ((uint32_t)y << 16) | x );
  sendCmd( ((uint32_t)h << 16) | w );
  sendCmd( ((uint32_t)val << 16) | options );
  sendCmd( (uint32_t)range );
}
...
{    
  sendCmd(CMD_SPINNER);
  sendCmd( ((uint32_t)y << 16) | x );
  sendCmd( ((uint32_t)scale << 16) | style );
}
...
void Cmd_Gauge(uint16_t x, uint16_t y, uint16_t r, uint16_t options, uint16_t major, uint16_t minor, uint16_t val, uint16_t range)
{
  sendCmd(CMD_GAUGE);
  sendCmd( ((uint32_t)y << 16) | x );
  sendCmd( ((uint32_t)options << 16) | r );
  sendCmd( ((uint32_t)minor << 16) | major );
  sendCmd( ((uint32_t)range << 16) | val );
}
...
void Cmd_Number(uint16_t x, uint16_t y, uint16_t font, uint16_t options, uint32_t num)
{
  sendCmd(CMD_NUMBER);
  sendCmd( ((uint32_t)y << 16) | x );
  sendCmd( ((uint32_t)options << 16) | font );
  sendCmd(num);
}
...
void Cmd_Gradient(uint16_t x0, uint16_t y0, uint32_t rgb0, uint16_t x1, uint16_t y1, uint32_t rgb1)
{
  ...
  sendCmd( ((uint32_t)y0<<16)|x0 );
  sendCmd(rgb0);
  sendCmd( ((uint32_t)y1<<16)|x1 );
  sendCmd(rgb1);
}
...
{ 
  ...
  sendCmd(CMD_BUTTON);
  sendCmd( ((uint32_t)y << 16) | x ); // Put two 16 bit values together into one 32 bit value - do it little endian
  sendCmd( ((uint32_t)h << 16) | w );
  sendCmd( ((uint32_t)options << 16) | font );
  
  for(LoopCount=0; LoopCount <= length/4; LoopCount++)
    sendCmd(data[LoopCount]);
}
...
void Cmd_Text(uint16_t x, uint16_t y, uint16_t font, uint16_t options, const char* str)
{
  ...
  // Send out the text
  for(LoopCount = 0; LoopCount <= length/4; LoopCount++)
    sendCmd(data[LoopCount]);  // These text bytes get sucked up 4 at a time and fired at the FIFO
}
...
void Cmd_SetBitmap(uint32_t addr, uint16_t fmt, uint16_t width, uint16_t height)
{
  ...
  sendCmd( addr );
  sendCmd( ((uint32_t)width << 16) | fmt );
  sendCmd( (uint32_t)height);
}
...
// These values determine where your calibration points will be drawn on your display
  ...
  while (count < numTouches) 
  {
    ...
    sendCmd(POINT_SIZE(20 * 16));
    sendCmd(BEGIN(POINTS));
    sendCmd(VERTEX2F((uint32_t)(displayX[count]) * 16, (uint32_t)((displayY[count])) * 16)); 
    sendCmd(END());
    ...
    while (pressed == count) {
      //Read for any new touch tag inputs and put a delay to avoid the same touch
...
// TODO: Make this return a bool by removing the do while and return
// ReadReg != WriteReg
...
{
	sendCmd(CMD_FLASHATTACH);
	updateFifo();                                                       // Trigger the CoProcessor to start processing commands out of the FIFO
	isCoProFifoEmpty();                                              // wait here until the coprocessor has read and executed every pending command.
